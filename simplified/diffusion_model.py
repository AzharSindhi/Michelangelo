...
from sita_vae import AlignedShapeAsLatentPLModule
...
class ClipASLDiffuser(pl.LightningModule):
    ...
    def __init__(self, *,
        ...
        self.first_stage_model = michelangelo.models.tsal.asl_pl_module.AlignedShapeAsLatentPLModule()
        self.first_stage_model.set_shape_model_only()

        # 2. initialize conditional stage
        self.cond_stage_model = michelangelo.models.conditional_encoders.encoder_factory.FrozenCLIPImageGridEmbedder()

        # 3. diffusion model
        self.model = michelangelo.models.asl_diffusion.asl_udt.ConditionalASLUDTDenoiser()
    ...
    def forward_diffusion(self, latents, conditions):
        """

        Args:
            latents (torch.FloatTensor): [bs, n_ctx, embed_dim]
            conditions (torch.FloatTensor): [bs, seq_len, context_dim]
    ...
    def training_step(self, batch: Dict[str, Union[torch.FloatTensor, List[str]]],
        ...
        # get point cloud encoded latent
        ...
        diffusion_output = self.forward_diffusion(latents, conditions)
        diffusion_loss = 0 # calculate diffusion here
        alignment_loss = 0 # caclulate alignment loss here
        # decoder 
        decoded_latent = self.decode_first_stage(latents, volume_queries)
        # reconstruct output
        
        combined_losses = diffusion_loss + alignment_loss
